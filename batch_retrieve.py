...
def _iter_anthropic_manifest_entries(path: Path) -> list[dict[str, str]]:
    ...
    with open(path, "r", encoding="utf-8") as handle:
        for line_number, line in enumerate(handle, start=1):
            ...
            custom_id = _normalize_key(payload.get("custom_id")) or _anthropic_custom_id_for_key(
                key
            )
            entries.append(
                {
...
def _submit_failed_pages_as_batch(
    *,
    ...
    if provider == "anthropic" or bool(getattr(client, "vertexai", False)):
        service_account_path = resolve_service_account_path(_require_service_account_file())
