...
def test_gaet_med_ulaeselig_optakt_bevarer_prikkerne():

    """`[..rede?]` er et gaet paa ordets slutning -- prikkerne er en del af gaettet."""

    tekst, _ = ren_laesetekst("Fra [..rede?] Saar")

    assert tekst == "Fra ..rede Saar"
...
def test_ukendt_klammeindhold_beholdes_som_tekst_og_flages():

    """`[gangrenerede]` er hverken maerke eller gaet -- vi gaetter ikke, vi flager."""

    tekst, noter = ren_laesetekst("tildels [gangrenerede] Belæg-")

    assert tekst == "tildels gangrenerede Belæg-"

    assert any("gangrenerede" in n for n in noter)





def test_uafsluttet_klamme_flages_og_teksten_bevares():

    """Fire filer har en tastefejl i opmaerkningen; teksten maa ikke gaa tabt."""

    tekst, noter = ren_laesetekst("Når der\n[ophentes? sekret af caviteten er dette")

    assert "sekret af caviteten er dette" in tekst
