...
Set `subagents=True` in `settings.py`, or enable **Sub Agent Usage** for an app job, to decompose each page into one batch request per top-level schema field. Every schema specialist searches the full processed page plus its OCR context, but receives only its one-field response schema and a compact prompt. Retrieval validates each specialist response, joins only pages with every specialist present, validates the full page schema, and then uses the normal dataset writer. The submit run records the mode and field list in `batch_job.json`; retrieve runs keep `subagent_combined.jsonl` and `subagent_failures.jsonl` as durable audit artifacts. This mode increases image/request count by the number of top-level schema fields; provider batch execution supplies the parallelism.
