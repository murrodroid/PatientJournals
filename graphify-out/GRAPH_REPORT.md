...
## Corpus Check
- 93 files · ~111,692 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1604 nodes · 4601 edges · 68 communities (57 shown, 11 thin omitted)
- Extraction: 96% EXTRACTED · 4% INFERRED · 0% AMBIGUOUS · INFERRED: 200 edges (avg confidence: 0.92)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `b2ef498c`
...
Cohesion: 0.06
Nodes (40): Connection, Row, Application services and desktop UI for PatientJournals., _copy_dataset_into_job(), _dataset_files(), JobStore, _json_dumps(), _json_loads() (+32 more)

### Community 1 - "tools.py"
Cohesion: 0.10
Nodes (44): ProgressCallback, main(), parse_args(), _progress_printer(), Namespace, _emit(), _input_without_existing(), LocalRunProgress (+36 more)

### Community 2 - "WorkflowService"
Cohesion: 0.16
Nodes (8): _apply_runtime_overrides(), _restore_runtime_overrides(), run_local_draft_direct(), command_override_payload(), Any, App-facing workflow API. Tk, web handlers, and tests should call this layer…, serializable(), WorkflowService
...
### Community 8 - "jobs.py"
Cohesion: 0.10
Nodes (58): _api_recovery_error_rows(), _api_recovery_error_summary(), _append_retry_child_to_source_metadata(), _batch_chunk_summaries_from_payload(), _batch_submit_namespace(), BatchSubmitOutcome, command_overrides_for_run(), _count_output_rows() (+50 more)
...
Cohesion: 0.16
Nodes (32): Fail before request generation when required cloud sidecars are unavailable., validate_ocr_metadata_for_blobs(), _batch_state_and_success(), _build_chunk_entry(), _build_rerun_entries(), _chunk_label(), _chunk_requests_file_name(), _discover_request_files_in_run_dir() (+24 more)

### Community 14 - "retrieve_batch"
Cohesion: 0.15
Nodes (18): RetrieveBatchResult, _effective_duplicate_strategy(), _expected_success_keys(), _extract_anthropic_response_metadata(), _extract_location_from_batch_name(), _failed_retry_num_batches(), _flush_rows(), _get_anthropic_client() (+10 more)

### Community 15 - "ui.py"
Cohesion: 0.11
Nodes (28): DuplicateStrategy, build_retrieve_command(), build_submit_command(), build_validation_command(), app_settings_path(), AppSettings, CommandSpec, Path (+20 more)
...
Cohesion: 0.18
Nodes (16): add_reproducibility_columns(), _failed_dataset_row(), _generate_recovery_response(), _guess_blob_mime_type(), _mark_success_rows(), Blob, _recover_one_missing_page_via_api_key(), generate_data() (+8 more)

### Community 20 - "catalog.py"
Cohesion: 0.12
...
Cohesion: 0.11
Nodes (41): collect_valid_outputs_from_jsonl_sources(), GeminiOutputParseResult, iter_gemini_jsonl_results(), normalize_output_key(), parse_gemini_output_record(), response_has_value(), _chosen_token_logprobs(), _collect_leaf_value_spans() (+33 more)

### Community 23 - "batch/service.py"
Cohesion: 0.13
Nodes (14): BatchChunkPlan, BatchCollectOutputsRequest, BatchSubmitPlan, BatchSubmitRequest, BatchSubmitService, Bucket, Namespace, _downscale_blobs_randomly() (+6 more)
...
Cohesion: 0.15
Nodes (22): cloud_object_by_image_name(), list_cloud_dataset_choices(), list_cloud_dataset_library(), list_cloud_dataset_prefixes(), resolve_local_images_on_cloud(), ImageAccessService, Any, Path (+14 more)
...
Cohesion: 0.24
Nodes (10): JobRegistry, list_app_registry_jobs(), list_cloud_batch_jobs(), _record_chunk_statuses_in_payload(), RegisteredJob, _retry_attempt_label(), start_command(), _summary_from_store_record() (+2 more)
...
Cohesion: 0.14
Nodes (7): build_validation_datapoints(), validation_sampling_group_key(), Validation UI and reporting commands., test_balanced_ucb_prioritizes_under_sampled_schema_field(), test_balanced_ucb_separates_missing_and_present_values(), test_random_sampling_uses_unvalidated_datapoints(), test_validation_candidates_are_schema_fields_only()
...
Cohesion: 0.12
Nodes (28): add_response_metadata_columns(), _api_key_recovery_failure_reason(), _await_completion(), _batch_job_state(), _batch_job_successful(), _build_api_key_generation_config(), _compact_exception_text(), _dataset_content_type() (+20 more)

### Community 59 - "collect_outputs.py"
Cohesion: 0.22
Nodes (18): Counter, collect_outputs(), CollectedGeminiOutputs, _counter_to_dict(), _expand_local_output_paths(), _flush_collected_rows(), _iter_blob_lines(), _list_page_image_names() (+10 more)
...
### Community 65 - "identity.py"
Cohesion: 0.20
Nodes (15): copy_dataset_rows_for_image_names(), copy_dataset_rows_for_keys(), load_dataset_image_coverage(), load_dataset_key_coverage(), normalize_dataset_image_name(), _normalize_output_format(), Path, resolve_continue_dataset_path() (+7 more)

### Community 66 - "workflows.py"
Cohesion: 0.26
Nodes (10): Retrieve a submitted batch in-process and record the result on the submit run.…, Resubmit the requests that did not succeed as a fresh batch. Clears the…, resubmit_failed_requests(), run_retrieve_direct(), _count_images(), _image_extensions(), _mtime(), Path (+2 more)
...
## Knowledge Gaps
- **43 isolated node(s):** `patientjournals`, `UploadProfile`, `Batch-first architecture`, `graphify`, `Answer` (+38 more)
...
- `LocalModelClient` (2× useful, score=1.998083926)

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `JobStore` connect `JobStore` to `WorkflowService`, `workflows.py`, `jobs.py`, `test_app_architecture.py`, `datasets.py`, `ui.py`?**
  _High betweenness centrality (0.065) - this node is a cross-community bridge._
- **Why does `PatientJournalsApp` connect `PatientJournalsApp` to `JobRegistry`, `datasets.py`, `ui.py`?**
  _High betweenness centrality (0.031) - this node is a cross-community bridge._
- **Why does `ValidatorApp` connect `ValidatorApp` to `validation/cli.py`, `image_name_from_reference`?**
