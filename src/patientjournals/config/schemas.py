...
class Serum(BaseModel):
    given: bool = Field(
        description="In the top left sometimes the text “serum” or similar is found."
    )
...
class PageLine(BaseModel):
    text: str = Field(
        description="This includes all text described that isn't written seperated from the line."
    )
    metadata: Optional[str] = Field(
        None,
        description="Contains the metadata of the line, can describe dates, temperatures, etc. Most commonly written in the left-side, before normal text is written.",
    )
...
class TextPage(BaseModel):
    page_lines: List[PageLine] = Field(
        description="This is meant for each line on the page, seperated by linebreaks."
    )
...
# Sparse second-pass model-validation output. These descriptions are kept here
# because they are part of the JSON Schema sent to the verifier model.
class ValidationIssuePatch(BaseModel):
    model_config = ConfigDict(extra="forbid")

    op: Literal["add", "replace", "remove"] = Field(
        description=(
            "RFC 6902 operation for one incorrect or missing candidate value. "
            "Use add for a missing value, replace for a wrong value, and remove "
            "only for candidate content unsupported by the page."
        )
    )
    path: str = Field(
        min_length=1,
        description=(
            "RFC 6901 JSON Pointer into the extraction candidate. It must start "
            "with '/'. Point to the smallest value that needs correction."
        ),
    )
    value_json: Optional[str] = Field(
        default=None,
        description=(
            "For add or replace, a compact JSON encoding of the replacement "
            "value, such as '\"Jensen\"', 'null', or '[\"A\",\"B\"]'. "
            "For remove, return null."
        ),
    )
    issue: Literal["incorrect", "missing", "unsupported"] = Field(
        description="The evidence problem corrected by this patch."
    )
    evidence: str = Field(
        min_length=1,
        max_length=400,
        description=(
            "Brief page-grounded evidence for the correction. Quote only the "
            "small relevant fragment or describe why the image cannot support it."
        ),
    )
    ocr_box_refs: List[str] = Field(
        default_factory=list,
        description=(
            "Zero or more supporting OCR boxes copied as 'x1,y1,x2,y2' from the "
            "supplied positional OCR. Keep empty when OCR has no useful match."
        ),
    )

    @model_validator(mode="after")
    def validate_patch_value(self) -> "ValidationIssuePatch":
        import json

        if not self.path.startswith("/"):
            raise ValueError("Validation patch path must be an RFC 6901 pointer.")
        for index, character in enumerate(self.path):
            if character != "~":
                continue
            if index + 1 >= len(self.path) or self.path[index + 1] not in {"0", "1"}:
                raise ValueError(
                    "Validation patch paths may use only RFC 6901 '~0' and '~1' escapes."
                )
        for box in self.ocr_box_refs:
            parts = box.split(",")
            if len(parts) != 4:
                raise ValueError(
                    "OCR box refs must contain four comma-separated integers."
                )
            try:
                tuple(int(part) for part in parts)
            except ValueError as exc:
                raise ValueError("OCR box refs must contain integers.") from exc
        if self.op == "remove":
            if self.value_json is not None:
                raise ValueError("Remove patches must set value_json to null.")
            return self
        if self.value_json is None:
            raise ValueError("Add and replace patches require value_json.")
        try:
...
class PageModelValidation(BaseModel):
    model_config = ConfigDict(extra="forbid")

    page_status: Literal["confirmed", "needs_correction", "unverifiable"] = Field(
        description=(
            "confirmed when every candidate field is supported and complete; "
            "needs_correction when one or more issue patches are required; "
            "unverifiable only when image evidence is insufficient to judge the page."
        )
    )
    patches: List[ValidationIssuePatch] = Field(
        default_factory=list,
        description=(
            "Only incorrect fields, as minimal non-overlapping patches. Return an "
            "empty list for confirmed or unverifiable pages; never repeat correct fields."
        ),
    )

    @model_validator(mode="after")
    def validate_status_and_patches(self) -> "PageModelValidation":
        if self.page_status == "needs_correction" and not self.patches:
            raise ValueError("needs_correction requires at least one patch.")
        if self.page_status != "needs_correction" and self.patches:
            raise ValueError(
                "Only needs_correction pages may contain validation patches."
            )
        paths = [patch.path for patch in self.patches]
        if len(paths) != len(set(paths)):
            raise ValueError("Validation patches must use unique JSON Pointer paths.")
        decoded_paths = [
            tuple(
        ...
        for index, left in enumerate(decoded_paths):
            for right in decoded_paths[index + 1 :]:
                shared = min(len(left), len(right))
                if left[:shared] == right[:shared]:
                    raise ValueError(
                        "Validation patches must not target an ancestor and its descendant."
                    )
        return self


MODEL_VALIDATION_SCHEMA = PageModelValidation.model_json_schema()
...
def model_from_json_schema(
    ...
    def field_info(node: dict[str, Any], default: object) -> Any:
        constraint_node, _nullable = unwrap(node)
        kwargs: dict[str, Any] = {}
        ...
        for source, target in constraint_names.items():
            if source in constraint_node:
                kwargs[target] = constraint_node[source]

        date_bound_names = {
            "formatMinimum": "ge",
            "formatMaximum": "le",
            "formatExclusiveMinimum": "gt",
            "formatExclusiveMaximum": "lt",
        }
        present_date_bounds = {
            source: constraint_node[source]
            for source in date_bound_names
            if source in constraint_node
        }
        if present_date_bounds:
            if not (
                constraint_node.get("type") == "string"
                and constraint_node.get("format") == "date"
            ):
                names = ", ".join(sorted(present_date_bounds))
                raise ValueError(
                    f"Managed JSON Schema date bound(s) {names} require "
                    "type='string' and format='date'."
                )
            for source, raw_bound in present_date_bounds.items():
                if not isinstance(raw_bound, str):
                    raise ValueError(
                        f"Managed JSON Schema {source} must be an ISO 8601 date string."
                    )
                try:
                    bound = date.fromisoformat(raw_bound)
                except ValueError as exc:
                    raise ValueError(
                        f"Managed JSON Schema {source} must be an ISO 8601 date string."
                    ) from exc
                kwargs[date_bound_names[source]] = bound
    ...
    def build_object(key: str, node: dict[str, Any]) -> type[BaseModel]:
        ...
        config_dict = ConfigDict(
            extra="forbid" if node.get("additionalProperties") is False else "ignore"
        )
        built = create_model(
