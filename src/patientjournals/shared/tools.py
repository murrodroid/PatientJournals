...
def list_input_files(cfg_obj: object) -> list[str]:
    ...
    if fp_mode == "exclude_fp":
        leaked = [p for p in files if _is_fp_file(p, folder, fp_suffix)]
        if leaked:
            raise RuntimeError(
                "Internal selection error: _fp files leaked in exclude_fp mode. "
                f"Example: {leaked[0]}"
            )
    ...
    if not files:
        raise FileNotFoundError(
            ...
            f"(pattern={pattern}, fp_mode={fp_mode}) "
...
def build_image_name_id_set(paths: list[str]) -> set[str]:
    return build_image_name_set(paths)


def filter_dataset_by_image_names(
    src_path: str | Path,
    dest_path: str | Path,
    image_names: set[str],
...
def filter_dataset_by_input_ids(
    ...
    return filter_dataset_by_image_names(
        src_path,
        dest_path,
